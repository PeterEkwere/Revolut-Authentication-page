...
export default function LoginForm() {
        ...
        fetch('https://restcountries.com/v3.1/all')
            ...
            .then((data) => {
                ...
                const formattedCountries = data.map((country) => ({
                    ...
                    flag: country.flags.png, // URL to the flag image
                    code: country.idd.root + (country.idd.suffixes ? country.idd.suffixes[0] : ''),
                }));

                const newCountryData = [...countriesData, ...formattedCountries]
                setCountries(newCountryData)

                // Set the default country to be the first from the newCountryData
                setSelectedCountry(newCountryData[0])
            })
    ...
    return (
        <div className="bg-[#f7f7f7] text-black w-full h-screen flex flex-col">
                ...
                <div className="min-h-[514px] mb-[10px] w-full max-w-[360px] flex flex-col">
                    ...
                    <div className="w-full">
                        <form action="" className="w-full sm:pr-[8px]">
                                ...
                                <div className='flex flex-row w-full gap-[0.5rem] py-[1rem]'>
                                    ...
                                    <div className='ovedrflow-hidden h-[56px] gap-x-2 flex w-full rounded-[1rem] caret-[#4f55f1] text-[#717173]'>
                                        <div className='w-[140px]'>
                                            <div className='flex cursor-pointer gap-x-2 items-center p-[16px] flex-row bg-[#ebebf0] h-full rounded-2xl'>
                                                {/* Country icon */}
                                                ...
                                                <div className='text-black'>
                                                    {selectedCountry?.code || 'N/A'}
                                                </div>
                                            </div>
                                        </div>

                                        {/* Number Input */}
                                        <div className="bg-[#ebebf0] flex justify-center px-[16px] h-full rounded-2xl flex-grow">
                                            <input
                                                type="number"
                                                placeholder="Phone number"
                                                className="w-full text-[#191c1f] appearance-none bg-[#ebebf0] rounded-2xl h-full focus:outline-none placeholder-[#191c1f98]"
                            ...
                            <div className='w-full flex justify-start inter font-normal text-[0.875rem] tracking-[-0.00714em] text-[#4f55f1]'>
                                Lost access to my phone number
                            </div>

                            <button disabled className='inter opacity-30 font-medium text-[1rem] tracking-[-0.01125em] h-[3.25rem] w-full min-w-[0px] px-[1rem] sm:mr-[1rem] rounded-[9999px] text-[white] bg-[#4f55f1] text-center my-[1.5rem]'>
                                Continue
                            </button>
                        </form>
            ...
            {/* footer */}
            <div className='h-[68px] justify-center md:justify-start md:flex-row flex-col flex px-[2rem] gap-y-2 gap-x-8 items-center md:pb-[8px] w-full bg-[#f7f7f7]'>
